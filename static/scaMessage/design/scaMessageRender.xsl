...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'scaMessage'">
        <xsl:call-template name="RenderscaMessage"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  
  <!-- scaMessage design render -->
  <xsl:template name="RenderscaMessage">
        ...
        <TR>
          <TD>
            <xsl:attribute name="background">
              ...
              <xsl:text>\design\message.png</xsl:text>
            </xsl:attribute>
          </TD>
        </TR>
      </table>
